...
pub type Map_{{k}}<'a> = Map<'a, {{k}}>;
pub type MapMut_{{k}}<'a> = MapMut<'a, {{k}}>;
{%- endfor %}
...
{{ cffi_gen_fn(k, type_mapping=type_mapping, **o).get("rust", "") }}
{% endfor %}

{% for k, o in cffi_mem_fns.items() -%}
{{ cffi_gen_mem_fn(k, type_mapping=type_mapping, **o).get("rust", "") }}
{% endfor %}
...
impl<'a> From<Map_{{k}}<'a>> for {{o.rs_ref}}<'a, {{o.tp}}> {
    fn from(v: Map_{{k}}<'a>) -> Self {
...
impl<'a> From<{{o.rs_ref}}<'a, {{o.tp}}>> for Map_{{k}}<'a> {
...
{%- endif %}
{% if o.rs_mut_ref -%}
impl<'a> From<MapMut_{{k}}<'a>> for {{o["rs_mut_ref"]}}<'a, {{o.tp}}> {
    fn from(v: MapMut_{{k}}<'a>) -> Self {
        // stride BUG strides: (CStride, RStride)
...
impl<'a> From<{{o.rs_mut_ref}}<'a, {{o.tp}}>> for MapMut_{{k}}<'a> {
...
impl<'a> From<&'a {{o.rs_own}}<{{o.tp}}>> for Map_{{k}}<'a> {
    fn from(v: &'a {{o.rs_own}}<{{o.tp}}>) -> Self {
        Self::new(v.coords.as_slice())
    }
}

impl<'a> From<Map_{{k}}<'a>> for {{o.rs_own}}<{{o.tp}}> {
    fn from(v: Map_{{k}}<'a>) -> Self {
        let d = unsafe {
...
impl<'a> From<&'a mut {{o.rs_own}}<{{o.tp}}>> for MapMut_{{k}}<'a> {
    fn from(v: &'a mut {{o.rs_own}}<{{o.tp}}>) -> Self {
        Self::new(v.coords.as_mut_slice())
    }
}

impl<'a> From<MapMut_{{k}}<'a>> for {{o.rs_own}}<{{o.tp}}> {
    fn from(v: MapMut_{{k}}<'a>) -> Self {
        let d = unsafe {
            std::slice::from_raw_parts(v.data, 4)
        };
